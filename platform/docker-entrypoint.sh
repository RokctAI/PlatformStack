  ...
  "full")
    ...
    if [ "$(id -u)" = "0" ]; then
      if [ "$SKIP_EXIM" != "1" ]; then
       ...
       # Inject ROK and Gravity location blocks into active Nginx config dynamically
       python3 -c '
import os
files = [
...
for f_path in files:
    if os.path.exists(f_path):
        ...
        modified = False
        if "location /rok/" not in content and "server {" in content:
            content = content.replace("server {", "server {" + rok_block, 1)
            modified = True
            print(f"Successfully injected ROK location block into {f_path}")
            
        if "location /gravity/" not in content and "server {" in content:
            content = content.replace("server {", "server {" + gravity_block, 1)
            modified = True
            print(f"Successfully injected Gravity location block into {f_path}")
            
        if modified:
            with open(f_path, "w") as f:
                f.write(content)
' || echo "⚠️ Nginx ROK/Gravity proxy injection failed, skipping"
