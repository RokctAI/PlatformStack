...
|---|-------|--------|------------|-------|
| 1 | Frontend & foundations | COVERED | RokctAI_frontend, paas_driver, shared-workflows | Next.js frontend and Flutter apps resolved dynamically; Clean Architecture boundary gates statically enforce decoupling of UI components from direct DB queries and raw API requests via `compliance_scanner.py` |
| 2 | APIs & backend logic | COVERED | rcore, rpanel, control | Orchestrator endpoints in [`control/api.py`](../control/control/api.py); AI & Strategic APIs in [`rcore/api/`](../rcore/rcore/api/); hosting APIs in [`rpanel/hosting/doctype/`](../rpanel/rpanel/hosting/doctype/) |
...
| 9 | Cloud, compute & containers | COVERED | PlatformStack, rpanel | Multi-stage Docker builds in [`Dockerfile`](platform/Dockerfile) & compose orchestration in [`docker-compose.yml`](platform/docker-compose.yml); SSH container bootstrapping in [`server_provisioner.py`](../rpanel/rpanel/hosting/server_provisioner.py); hub-to-tenant reachability is confined to the shared external `rokct_tenants` bridge (hub nginx proxies `https://<custom domain>` to `http://<site_name>-app:8000`, the VPS-hosted shell to `http://juvo-shell:3000`; no tenant or shell port is published to the host) with certbot in the hub image and `/etc/letsencrypt` persisted, see the README section "Tenant backend domains" |
| 10 | Hosting & deployment | COVERED | PlatformStack, rpanel, shared-workflows | Bench synthesis and patching in [`build_ecosystem.sh`](platform/scripts/build_ecosystem.sh); remote server setups in [`server_provisioner.py`](../rpanel/rpanel/hosting/server_provisioner.py); plain IP leak detection and Localhost Decoupling Gate actively enforced in CI to keep local and remote spoke environments perfectly identical |
